...
public class ParserJob extends NutchTool implements Tool{
    ...
    public static class ParserMapper extends GoraMapper<String,WebPage,String,WebPage> {
        ...
        protected void map(String key, WebPage page, Context context) throws IOException, InterruptedException {
           ...
           parseUtil.process(key, page);
            ...
            if (status != null) {
                context.getCounter("ParserStatus",ParseStatusCodes.majorCodes[status.getMajorCode()]).increment(1);
            }
    ...
    public static boolean isTruncated(String url, WebPage page) {
        ...
        CharSequence lengrhUtf8 = page.getHeaders().get(new Utf8(HttpHeaders.CONTENT_TYPE));
        if (lengrhUtf8 == null) {
            return false;
        }
        String lengthStr = lengrhUtf8.toString().trim();
        ...
        if (inHeaderSize > actualSize) {
            LOG.warn(url + "skipped. Context of size " + inHeaderSize + " was truncated to " + actualSize);
            return true;
        }
        if (LOG.isDebugEnabled()) {
    ...
    public int parse(String batchId, boolean shouldResume, boolean force) throws Exception {
        ...
        run(ToolUtil.toArgMap(Nutch.ARG_BATCH, batchId, Nutch.ARG_RESUME,shouldResume,Nutch.ARG_FORCE,force));
